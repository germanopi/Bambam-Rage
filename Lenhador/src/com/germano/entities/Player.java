...
public class Player extends Entity {
	...
	public Player(int x, int y, int width, int height, BufferedImage sprite) {
		...
		for (int i = 0; i < 2; i++) {
			playerDamage[i] = Game.spritesheet.getSprite(i * 16, 16, 16, 16);
		}
	...
	public void render(Graphics g) {
		...
		if (!isDamaged) {
			...
			if (dir == right_dir) {
				...
				if (hasWeapon) {
					g.drawImage(Entity.GUN_RIGHT_EN, this.getX() + 8 - Camera.x, this.getY() + 5 - Camera.y - z, null);
				}
				...
				if (hasWeapon) {
					g.drawImage(Entity.GUN_LEFT_EN, this.getX() - 8 - Camera.x, this.getY() + 5 - Camera.y - z, null);
				}
			}
			// Se levou dano
		} else {
			// desenha jogador para direita levando dano
			if (dir == right_dir) {
				g.drawImage(playerDamage[0], this.getX() - Camera.x, this.getY() - Camera.y - z, null);
				// desenha arma para direita levando dano
				if (hasWeapon) {
					g.drawImage(Entity.GUN_RIGHT_WHITE_EN, this.getX() + 8 - Camera.x, this.getY() + 5 - Camera.y - z,
							null);
				}
				// desenha jogador para esquerda levando dano
			} else if (dir == left_dir) {
				g.drawImage(playerDamage[1], this.getX() - Camera.x, this.getY() - Camera.y - z, null);
				// desenha arma para esquerda levando dano
				if (hasWeapon) {
					g.drawImage(Entity.GUN_LEFT_WHITE_EN, this.getX() - 8 - Camera.x, this.getY() + 5 - Camera.y - z,
							null);
				}
			}
		}
