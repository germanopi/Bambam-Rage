...
public class AStar {
	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end) {
		...
		List<Node> openList = new ArrayList<Node>(); // Lista de posições possiveis
		List<Node> closedList = new ArrayList<Node>(); // Lista de posições impossiveis
		...
		while (openList.size() > 0) {
			...
			for (int i = 0; i < 9; i++) {// Verifica todas as posições vizinhas a atual
				if (i == 4) { // Posição do inimigo
					continue;
				}
				...
				// Se for uma posição impossivel e o custo do caminho for maior que um calculado
	...
	// Verifica se o Node já está na lista
	...
	// Calcula a distancia entre um tile e uma posição
